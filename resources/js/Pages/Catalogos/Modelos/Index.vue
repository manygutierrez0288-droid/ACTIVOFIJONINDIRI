...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('modelos.update', itemId.value), {
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(route('modelos.store'), {
            onSuccess: () => closeModal(),
        });
    }
};
...
const deleteItem = () => {
    router.delete(route('modelos.destroy', itemToDelete.value), {
        onSuccess: () => {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Modelos</h2>
        </template>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr v-for="item in modelos.data" :key="item.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ item.nombre }}
                                    </td>
                                    <td class="px-6 py-4">
                                        {{ item.marca_nombre || 'N/A' }}
                                    </td>
