...
const props = defineProps({ modelo: Object, marcas: Array });
...
const submit = () => form.put(route('modelos.update', props.modelo.id));
</script>

<template>
    <Head title="Editar Modelo" />
    <AuthenticatedLayout>
        <template #header><h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar Modelo</h2></template>
        <div class="py-12"><div class="max-w-7xl mx-auto sm:px-6 lg:px-8"><div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
            <form @submit.prevent="submit">
                ...
                <div class="flex justify-end mt-4"><Link :href="route('modelos.index')" class="mr-4 underline text-gray-600">Cancelar</Link><PrimaryButton :disabled="form.processing">Actualizar</PrimaryButton></div>
            </form>
        </div></div></div>
    </AuthenticatedLayout>
</template>
