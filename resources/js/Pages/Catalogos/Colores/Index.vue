...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('colores.update', itemId.value), {
...
const deleteItem = () => {
    router.delete(route('colores.destroy', itemToDelete.value), {
        onSuccess: () => {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Colores</h2>
        </template>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Muestra</th>
                                    <th scope="col" class="px-6 py-3 text-right">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="item in colores.data" :key="item.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ item.nombre }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div :style="{ backgroundColor: item.hex_code || item.nombre }" class="w-8 h-8 rounded-full border-2 border-white shadow-sm ring-1 ring-gray-200"></div>
                                    </td>
        ...
        <Modal :show="showingModal" @close="closeModal">
            <div class="p-6">
                ...
                <form @submit.prevent="submitForm" class="mt-6 space-y-6">
                    <div>
                        <InputLabel for="nombre" value="Nombre del Color" />
                        <TextInput
                            id="nombre"
                            type="text"
                            ...
                            required
                            autofocus
                            placeholder="Ej: Rojo, Azul Marina, #FF0000"
                        />
                        <p class="text-xs text-gray-500 mt-1 italic">Puedes ingresar el nombre del color o su código hexadecimal.</p>
