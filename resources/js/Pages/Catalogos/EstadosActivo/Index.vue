...
const submitForm = () => {
    if (isEditing.value) {
        form.put(route('estados.update', itemId.value), {
...
const deleteItem = () => {
    router.delete(route('estados.destroy', itemToDelete.value), {
        onSuccess: () => {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Estados de Activo</h2>
        </template>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Badge Color</th>
                                    <th scope="col" class="px-6 py-3 text-right">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="item in estados.data" :key="item.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ item.nombre }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span :style="{ backgroundColor: item.color_hex }" class="px-3 py-1 rounded-full text-white text-xs font-bold shadow-sm">
                                            {{ item.color_hex || 'N/A' }}
                                        </span>
                                    </td>
        ...
        <Modal :show="showingModal" @close="closeModal">
            <div class="p-6">
                ...
                <form @submit.prevent="submitForm" class="mt-6 space-y-6">
                    <div>
                        <InputLabel for="nombre" value="Nombre del Estado" />
                        <TextInput
                            id="nombre"
                            type="text"
                    ...
                    <div class="flex items-center justify-end">
                        <0SecondaryButton @click="closeModal" class="mr-3">
                            Cancelar
                        </0SecondaryButton>
